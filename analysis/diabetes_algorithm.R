diabetes_algo <- function(df){
  df <- df %>%
    ...
    mutate(step_5 = ifelse(step_4 == "No" &
                             age_under_35_30_1st_diag == "Yes", "Yes", # includes many codes: cov_date_t2dm, cov_date_t1dm, cov_date_otherdm, cov_date_gestationaldm, cov_date_poccdm, tmp_cov_date_diabetes_medication, tmp_cov_date_nonmetform_drugs_snomed
                           ifelse(step_4 == "No" &
                                    age_under_35_30_1st_diag == "No", "No", NA))) %>%
    mutate(step_5 = ifelse(step_5 == "No" |
                             is.na(step_5) & step_4 == "No", "No", "Yes")) %>% ### shouldn't this step also be included above for the other Steps in case of step is.na in order to carry such a person forward in the algo?
    ...
    mutate(step_6a = ifelse(step_6 == "Yes" &
                              !is.na(tmp_cov_date_t1dm_snomed) &
                              is.na(tmp_cov_date_t2dm_snomed), "Yes",
    ...
    mutate(step_6c = ifelse(step_6b == "No" &
                            ...
                            ifelse(step_6b == "No" &
                                     tmp_cov_count_t1dm <= tmp_cov_count_t2dm, "No", NA))) %>%
    ...
    mutate(step_6d = ifelse(step_6c == "No" &
                            ...
                            ifelse(step_6c == "No" &
                                     tmp_cov_count_t2dm <= tmp_cov_count_t1dm, "No", NA))) %>%
    ...
    mutate(step_6e = ifelse(step_6d == "No" &
                            ...
                            ifelse(step_6d == "No" &
                                     cov_date_t2dm < cov_date_t1dm, "No", NA))) %>%
    ...
    mutate(step_7 = ifelse(step_6 == "No" &
                             ((!is.na(tmp_cov_date_diabetes_medication)) |
                                ...
                                (tmp_cov_count_poccdm_snomed >= 5)), "Yes",
    ...
    # GESTATIONAL
    mutate(cov_date_gestationaldm = as_date(case_when(cov_cat_diabetes == "GDM" ~ tmp_cov_date_first_diabetes_diag)),
           # T2DM
           cov_date_t2dm = as_date(case_when(cov_cat_diabetes == "T2DM" ~ tmp_cov_date_first_diabetes_diag)),
           # T1DM
           cov_date_t1dm = as_date(case_when(cov_cat_diabetes == "T1DM" ~ tmp_cov_date_first_diabetes_diag)),
           # OTHER
